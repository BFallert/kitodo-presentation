    ...
    protected function buildSimplePagination(PaginationInterface $pagination, PaginatorInterface $paginator): array
    {
        ...
        foreach (range($firstPage, $lastPage) as $i) {
            ...
            if (get_class($pagination) == 'TYPO3\CMS\Core\Pagination\SimplePagination') { // ListView
                ...
                $startRecordNumber = $itemsPerPage * $i + 1 - $itemsPerPage;
                $lastStartRecordNumberGrid = $startRecordNumber; // save last startRecordNumer for LastPage button
                // array with label as screen/pagination page number and startRecordNumer for correct structure of the link
        ...
        $nextPageNumber = $pages[$currentPageNumber + 1]['startRecordNumber'] ?? null;
        $previousPageNumber = $pages[$currentPageNumber - 1]['startRecordNumber'] ?? null;

        return [
            'lastPageNumber' => $lastPage,
            'lastPageNumberG' => $lastStartRecordNumberGrid ?? null,
